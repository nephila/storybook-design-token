...
function parseCssFiles(files, sourceType, injectVariables) {
    if (files === void 0) { files = []; }
    return __awaiter(this, void 0, void 0, function () {
        var nodes, categories, injectionStyles;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, getNodes(files.filter(function (file) { return file.content; }))];
                case 1:
                    nodes = _a.sent();
                    categories = determineCategories(nodes.comments, nodes.declarations, sourceType);
...
function determineCategories(comments, declarations, sourceType) {
    var categoryComments = comments.filter(function (comment) {
        return comment.text.includes('@tokens ');
    });
    return categoryComments.map(function (comment, index) {
        ...
        var source = ((_l = comment.source) === null || _l === void 0 ? void 0 : _l.input.from) || '';
        return {
            name: (nameResults === null || nameResults === void 0 ? void 0 : nameResults[1]) || '',
            presenter: presenter,
            range: range,
            source: source,
            tokens: determineTokensForCategory(source, range, declarations, comments, sourceType, presenter)
        };
    });
}
function determineTokensForCategory(source, range, declarations, comments, sourceType, presenter) {
    ...
    return declarationsWithinRange.map(function (declaration) {
        ...
        var value = determineTokenValue(declaration.value, declarations);
        var presenterToken;
        if (description) {
...
function determineTokenValue(rawValue, declarations) {
    var _a;
    rawValue = rawValue.replace(/!default/g, '').replace(/!global/g, '');
    var referencedVariableResult = /^((var\(([a-zA-Z0-9-_]+)\))|(\$([a-zA-Z0-9-_]+))|(\@([a-zA-Z0-9-_]+)))$/.exec(rawValue);
            ...
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, Promise.all(files.map(function (file) {
                            var syntax = file.filename.endsWith('.scss') ? postcss_scss_1.default : undefined;
                            return postcss_1.default([plugin]).process(file.content, {
                                from: file.filename,
                                syntax: syntax
                            });
                        }))];
